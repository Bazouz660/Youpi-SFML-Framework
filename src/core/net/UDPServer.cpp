...
namespace ypi::net {
    ...
    UDPServer::UDPServer()
    {
        ...
        registerPacketHanlder(0, [](Packet &packet, UDPServer &server, const Client &client) {
            // check last time a ping packet was received from the client
        });
    }
    ...
    void UDPServer::run()
    {
        while (!m_shouldClose) {
            ...
            if (m_chrono.getElapsedTime().asSeconds() > m_tickRate) {
                // send a ping packet to all clients to check for connection liveliness
                notifyPacket(Packet(0));

    ...
    private:
        ...
        Rect<Float> computeBox(const Rect<Float>& box, int i) const
        {
            ...
            switch (i)
            {
                ...
                // Norst East
                case 1:
                    return Rect<Float>(Vector2<Float>(origin.x + childSize.x, origin.y), childSize);
                // South West
                case 2:
                    return Rect<Float>(Vector2<Float>(origin.x, origin.y + childSize.y), childSize);
                // South East
                case 3:
        ...
        bool remove(Node* node, const Rect<Float>& box, const T& value)
        {
            assert(node != nullptr);
            assert(box.contains(mGetBox(value)));
            if (isLeaf(node))
